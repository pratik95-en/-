...
function applyTheme(t, save=true) {
  ...
  // (theme button removed from header - only in 3-dots menu)
...
/* ════════════════════════════════════
   ...
   ════════════════════════════════════ */
const FONTS = {
  'noto-serif':  { label: 'Noto Serif',   css: "'Noto Serif Devanagari', serif" },
  'eczar':       { label: 'Eczar',        css: "'Eczar', serif" },
  'khand':       { label: 'Khand',        css: "'Khand', sans-serif" },
  'gotu':        { label: 'Gotu',         css: "'Gotu', sans-serif" },
  'comic-sans':  { label: 'Comic Sans',   css: "'Comic Sans MS', cursive" },
  'chakra':      { label: 'Chakra Petch', css: "'Chakra Petch', sans-serif" },
};
...
function applyFont(fontId, save=true) {
  ...
  if (f) {
    ...
    // For Devanagari fonts, also update --font-s
    if (['noto-serif','eczar','khand','gotu'].includes(fontId)) {
      document.documentElement.style.setProperty('--font-s', f.css);
    } else {
      // English fonts: keep Noto Serif for Devanagari display text
...
function setNews(idx) {
  ...
  if (anim === 'slide') {
    ...
    track.style.transform = 'translateX(-' + (idx * (cards[0].offsetWidth + 12)) + 'px)';
    track.style.transition = 'none';
    cards.forEach((c, i) => {
      c.style.transition = 'opacity 0.45s ease';
      c.style.opacity = i === idx ? '1' : '0.15';
    });
    ...
    cards.forEach((c, i) => {
      c.style.transition = 'transform 0.45s cubic-bezier(0.34,1.56,0.64,1), opacity 0.3s ease';
      c.style.transform = i === idx ? 'scale(1)' : 'scale(0.9)';
